...
import { botString, productTpl } from '@/lib/chatbot-strings';
...
function generateBotResponse(input: string, locale: string): string {
  ...
  for (const p of products) {
    ...
    if (q.includes(name.split(' ')[0].toLowerCase()) || q.includes(p.slug.split('-')[0])) {
      ...
      const stockKey =
        p.stockStatus === 'in_stock' ? 'stock_in'
        : p.stockStatus === 'low_stock' ? 'stock_low'
        : 'stock_out';
      const tpl = productTpl(locale);
      return tpl(
        p.name[locale] || p.name.en,
        fmt(price),
        botString(stockKey, locale),
